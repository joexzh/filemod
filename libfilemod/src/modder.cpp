...
static void set_fail(result_base& ret,
                     std::initializer_list<std::string_view> vs) {
  ret.success = false;
  for (auto v : vs) {
    ret.msg += v;
  }
...
static void set_fail(result_base& ret, std::string_view v) {
  ret.success = false;
  ret.msg = v;
}
...
static bool fail_if_not_directory(result_base& ret,
...
static bool fail_if_not_exists(result_base& ret,
                               const std::filesystem::path& path,
                               std::string_view pathv) {
...
result<int64_t> modder::add_target(std::string_view tar_dir_raw) {
  ...
  tx_wrapper_(ret, [&]() {
    if (auto tar_ret = m_db.query_target_by_dir(tar_dir); tar_ret.success) {
      ret.data = tar_ret.data.id;
      // if target exists, do nothing
      return;
    }

    ret.data = m_db.insert_target(tar_dir);
...
result<int64_t> modder::add_mod_(int64_t tar_id, std::string_view mod_name,
...
result<int64_t> modder::add_mod(int64_t tar_id, std::string_view mod_name,
  ...
  return add_mod_(tar_id, mod_name, mod_dir_stripped, copy_mod);
}
...
result_base modder::install_mod_(int64_t mod_id) {
  ...
  tx_wrapper_(ret, [&]() {
    ...
    auto cfg_mod_path = m_fs.get_cfg_mod_path(mod.tar_id, mod.dir);
    ...
    if (auto conflict_mods = find_conflict_mods(cfg_mod_path, mod, m_db);
        !conflict_mods.empty()) {
      set_fail(ret,
               std::string_view{
    ...
    auto bak_file_rel_paths = m_fs.install_mod(cfg_mod_path, tar_dir_path);
    ...
    m_db.install_mod(mod.id, bak_file_rels);
  });
...
result<int64_t> modder::install_mod_path(int64_t tar_id,
  ...
  return install_mod_path_(tar_id, mod_name, mod_dir_stripped,
                           &modder::add_mod);
}
...
result<int64_t> modder::install_mod_path_(int64_t tar_id,
                                          std::string_view mod_name,
...
result<ModDto> modder::uninstall_mod_(int64_t mod_id) {
  ...
  tx_wrapper_(ret, [&]() {
    ...
    if (tar_ret.success == true) {
      ...
      m_fs.uninstall_mod(m_fs.get_cfg_mod_path(mod.tar_id, mod.dir),
...
result_base modder::remove_mod_(int64_t mod_id) {
  ...
  tx_wrapper_(ret, [&]() {
    ...
    m_fs.remove_mod(m_fs.get_cfg_mod_path(unin_mod.tar_id, unin_mod.dir));
  });
...
result_base modder::rename_mod(int64_t mid, std::string_view newname) {
  ...
  tx_wrapper_(ret, [&]() {
    ...
    m_fs.rename_mod(oldmod.tar_id, oldmod.dir, newname);
  });
